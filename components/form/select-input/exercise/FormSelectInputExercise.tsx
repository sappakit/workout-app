...
export interface FormSelectInputProps<T = unknown> {
...
export default function FormSelectInputExercise<T = unknown>({
  options,
  value,
  onChange,
  ...
  const renderItem: ListRenderItem<SelectOption<T>> = ({ item, index }) => {
      ...
      >
        ...
        {isLoading || isError || options.length === 0 ? (
          ...
          <BottomSheetFlatList
            ref={listRef}
            data={options}
            keyExtractor={(item: SelectOption<T>) => item.value.toString()}
